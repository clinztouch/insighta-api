...
let ProfilesService = class ProfilesService {
    ...
    async getProfiles(query, baseUrl = '/api/profiles') {
        ...
        if (query.age_group)
            params.set('age_group', query.age_group);
        if (query.country_id)
            params.set('country_id', query.country_id);
        if (query.min_age)
            params.set('min_age', String(query.min_age));
        if (query.max_age)
    ...
    async searchProfiles(q, query) {
        ...
        const filters = {};
        if (/\bmales?\b/.test(input))
            filters.gender = 'male';
        else if (/\bfemales?\b/.test(input))
            filters.gender = 'female';
        else if (/\bmen\b/.test(input))
            filters.gender = 'male';
        ...
        if (olderMatch)
            filters.min_age = parseInt(olderMatch[1]);
        if (youngerMatch)
            filters.max_age = parseInt(youngerMatch[1]);
        if (betweenMatch) {
    ...
    async createProfile(name) {
        const [genderRes, nationalizeRes] = await Promise.all([
            axios_1.default.get(`https://api.genderize.io/?name=${encodeURIComponent(name)}`),
            axios_1.default.get(`https://api.nationalize.io/?name=${encodeURIComponent(name)}`),
        ]);
